...
contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x8dC778E2604C4E0f68ed7D516f00ec1140Ae4121;//0x8dC778E2604C4E0f68ed7D516f00ec1140Ae4121;  // 
...
contract RedLightCoin  is Ownable, IERC20, IERC20Metadata {
    ...
    uint256 public maxTxAmount = _totalSupply; // no limit
    ...
    address marketingWallet=0xdbD3a0b24eDF56C43420f3e9DdB17f30D4fAf9d2;
    
    uint256 marketingFee=300; //marketing fee 3%
    uint256 liquidityFee=200; // liquidity fee 2%
    ...
    function findMaketingPercent(uint256 value) internal view returns (uint256)  {
    ...
    function findLiquidityPercent(uint256 value) internal view returns (uint256)  {
    uint256 roundValue = value.ceil(liquidityFee);
    uint256 onePercent = roundValue.mul(liquidityFee).div(10000);
    return onePercent;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient]){
    ...
    function collectFee(address account, uint256 amount) private returns (uint256) {
        ...
        if(marketingFee != 0){
             //1
            uint256 _makretingFee=findMaketingPercent(transferAmount);
        ...
        if (liquidityFee != 0){
            uint256 _liquidityFee=findLiquidityPercent(transferAmount);
